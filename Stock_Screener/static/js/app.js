...
async function runScreener() {
  ...
  // Warn if scanning all NSE (slow)
  if (state.universe === 'all_nse') {
    showToast('⚠ Scanning all 2,250+ NSE stocks — this may take 5–15 minutes.', 'warning');
  }
  ...
  try {
    ...
    // Store results
    state.results    = data.stocks || [];
    state.sectorData = data.sector_heatmap || {};

    // Update sidebar meta
    const meta = document.getElementById('screen-meta');
    if (meta) {
      meta.textContent = `✓ Scanned ${data.screened ?? '—'} · Found ${state.results.length}`;
